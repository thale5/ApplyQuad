...
namespace TerraformTool
{
    public class InGameTerrainTool : ToolBase
    {
        ...
        protected override void OnToolGUI(Event current)
        {
        ...
        public override void SimulationStep()
        {
            ...
            else if (this.m_mouseRayValid && ToolBase.RayCast(input, out raycastOutput))
            {
                ...
                if (this.m_mouseLeftDown != this.m_mouseRightDown)
                {
                    ...
                    // ApplyBrush();
                    ApplyTest(m_mousePosition, m_startPosition);
                }
            }
        }
        ...
        private void ApplyTest(Vector3 mousePos, Vector3 startPos)
        {
            try
            {
                MyManager.x1 = mousePos.x - m_brushSize;
                MyManager.x2 = mousePos.x + m_brushSize * 1.5f;
                MyManager.x3 = mousePos.x + m_brushSize;
                MyManager.x4 = mousePos.x - m_brushSize;

                MyManager.y = startPos.y;

                MyManager.z1 = mousePos.z + m_brushSize;
                MyManager.z2 = mousePos.z + m_brushSize * 1.8f;
                MyManager.z3 = mousePos.z - m_brushSize;
                MyManager.z4 = mousePos.z - m_brushSize;

                MyManager.available = true;

                // Notice that all three booleans are true here:
                TerrainModify.UpdateArea(MyManager.x1, MyManager.z4, MyManager.x2, MyManager.z2, true, true, true);
            }
            catch (Exception ex)
            {
                Debug.Log("ApplyTest");
    ...
    public class MyManager : ITerrainManager
    {
        public static float x1, x2, x3, x4;
        public static float z1, z2, z3, z4;
        ...
        public void TerrainUpdated(TerrainArea heightArea, TerrainArea surfaceArea, TerrainArea zoneArea)
        {
            ...
            try
            {
                Vector3 a1 = new Vector3(x1, y, z1);
                Vector3 a2 = new Vector3(x2, y, z2);
                Vector3 a3 = new Vector3(x3, y, z3);
                Vector3 a4 = new Vector3(x4, y, z4);

                // Modifies the height using the detail map (4x4 m resolution).
                // TerrainModify.ApplyQuad(a1, a2, a3, a4, TerrainModify.Edges.All, TerrainModify.Heights.PrimaryLevel | TerrainModify.Heights.SecondaryLevel | TerrainModify.Heights.PrimaryMax |
                //     TerrainModify.Heights.BlockHeight | TerrainModify.Heights.SecondaryMin, TerrainModify.Surface.None);


                // Just the pavement
                TerrainModify.ApplyQuad(a1, a2, a3, a4, TerrainModify.Edges.All, TerrainModify.Heights.None, TerrainModify.Surface.PavementA);
            }
            catch (Exception ex)
            {
                Debug.Log("MyManager");
